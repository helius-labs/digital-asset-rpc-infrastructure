...
use crate::{
    ...
    programs::{token_extensions::t22_parsing_fix::get_variable_len_extension, ProgramParseResult},
};
...
use self::extension::{
    ...
    ShadowMetadata, ShadowMetadataPointer, ShadowMintCloseAuthority, ShadowPausableConfig,
    ShadowPermanentDelegate, ShadowScaledUiAmountConfig, ShadowTokenGroup,
    ShadowTokenGroupMember, ShadowTransferFeeAmount, ShadowTransferFeeConfig, ShadowTransferHook,
};
...
pub struct MintAccountExtensions {
    ...
    pub pausable_config: Option<ShadowPausableConfig>,
    pub scaled_ui_amount_config: Option<ShadowScaledUiAmountConfig>,
}
...
pub struct TokenExtensionsAccountParser;
...
impl ProgramParser for TokenExtensionsAccountParser {
    fn key(&self) -> Pubkey {
        token_program_id()
    }
    ...
    fn handle_account(
        ...
        if let Ok(account) = StateWithExtensions::<Account>::unpack(&account_data) {
            ...
            let metadata_pointer = mint
                ...
                .ok()
                .map(|x| x.clone());
            let metadata = get_variable_len_extension::<TokenMetadata>(&mint)
                .ok()
                .map(|x| x.clone());
            ...
            let group_member_pointer = mint
                ...
                .ok()
                .map(|x| x.clone());
            let token_group_member = mint.get_extension::<TokenGroupMember>().ok().map(|x| x.clone());
            let transfer_hook = mint.get_extension::<TransferHook>().ok().map(|x| x.clone());
            let pausable_config = mint.get_extension::<PausableConfig>().ok().map(|x| x.clone());
            let scaled_ui_amount_config = mint.get_extension::<ScaledUiAmountConfig>().ok().map(|x| x.clone());
            ...
            let structured_mint = MintAccount {
                ...
                extensions: MintAccountExtensions {
                    ...
                    pausable_config: pausable_config.map(ShadowPausableConfig::from),
                    scaled_ui_amount_config: scaled_ui_amount_config.map(ShadowScaledUiAmountConfig::from),
                },
            };
            result = TokenExtensionsProgramAccount::MintAccount(structured_mint);
        } else if let Ok(metadata) = TokenMetadata::unpack_from_slice(&account_data) {
            let structured_metadata = TokenMetadataAccount {
                metadata: ShadowMetadata::from(metadata),
            };
            result = TokenExtensionsProgramAccount::TokenMetadataAccount(structured_metadata);
        } else {
