use crate::{
    ...
    program_transformers::{
        ...
        token::upsert_owner_for_account,
    },
};
...
pub async fn handle_token_extensions_token_account<'a, 'b, 'c>(
    ta: &TokenAccount,
    ...
    let extensions = serde_json::to_value(ta.extensions.clone())
        ...
        .filter(
            asset::Column::OwnerType.eq("single").and(
                asset::Column::SlotUpdatedTokenAccount
                    .is_null()
                    .or(asset::Column::SlotUpdatedTokenAccount.lte(account_update.slot() as i64)),
            ),
        )
        .one(&txn)
        .await?;
    ...
    if let Some(asset) = asset_update {
        ...
        if asset_clone.supply == 1 && ta.account.amount > 0 {
            let mut save_required = false;
            let mut active: asset::ActiveModel = asset.into();

            // Handle ownership updates
            // IMPORTANT: Always update owner if it's NULL or different, to fix race condition
            // where mint account is processed before token account
            let old_owner = asset_clone.owner.clone();
            let new_owner = owner.clone();
            if old_owner.is_none() || Some(new_owner) != old_owner {
            ...
            if delegate.clone() != asset_clone.delegate {
            ...
            if save_required {
                upsert_assets_token_account_columns(
                    AssetTokenAccountColumns {
                        ...
                        frozen,
                        delegate,
                        token_extensions: Some(token_extensions),
