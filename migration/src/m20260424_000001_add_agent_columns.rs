...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // Use CONCURRENTLY to avoid blocking writes on the 1B+ row asset
        // table. These are partial indexes that match 0 rows on first deploy
        // (all values are false/NULL), so the resulting B-trees are tiny.
