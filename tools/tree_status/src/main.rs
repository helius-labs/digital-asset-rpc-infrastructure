...
async fn find_signatures_for_missing_seq_range(
    ...
    let before_txn = cl_audits_v2::Entity::find()
        .filter(cl_audits_v2::Column::Tree.eq(tree.as_ref()))
        .filter(cl_audits_v2::Column::Seq.gte(end))
        .order_by_asc(cl_audits_v2::Column::Seq)
        .limit(1)
        .all(conn)
        .await?;
    let before_txn = before_txn.first().map(|m| m.tx.clone());

    // Find the indexed seq before the start of the range.
    let until_txn = cl_audits_v2::Entity::find()
        .filter(cl_audits_v2::Column::Tree.eq(tree.as_ref()))
        .filter(cl_audits_v2::Column::Seq.lte(start))
        .order_by_desc(cl_audits_v2::Column::Seq)
        .limit(1)
        .all(conn)
        .await?;
    let until_txn = until_txn.first().map(|m| m.tx.clone());
    ...
    trace!(
        ...
        until_txn
            .as_ref()
            .map_or("None".to_string(), |txn| bs58::encode(txn.clone())
                .into_string()),
        before_txn
            .as_ref()
            .map_or("None".to_string(), |txn| bs58::encode(txn.clone())
    ...
    let mut before = before_txn.map(|t| Signature::try_from(t).unwrap());
    let until = until_txn.map(|t| Signature::try_from(t).unwrap());
...
async fn get_tree_max_seq(
    ...
    let query = cl_audits_v2::Entity::find()
        .select_only()
        .filter(cl_audits_v2::Column::Tree.eq(tree.as_ref()))
        .column_as(Expr::col(cl_audits_v2::Column::Seq).max(), "max_seq")
...
WHERE
    NOT EXISTS (
        SELECT 1 FROM cl_audits_v2 WHERE seq = s.seq AND tree='\\x{}'
    )",
            max_seq,
