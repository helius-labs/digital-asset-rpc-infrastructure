...
WORKDIR /
# Copy in local repo of plerkle and blockbuster
COPY rust-services/das-system/ /das-system/
WORKDIR /das-system/util-jobs
# Build application
...
RUN groupadd $APP_USER \
    ...
    && mkdir -p ${APP}
COPY --from=builder /das-system/target/release/util-jobs ${APP}
RUN chown -R $APP_USER:$APP_USER ${APP}
USER $APP_USER
WORKDIR ${APP}
CMD /usr/src/app/util-jobs
